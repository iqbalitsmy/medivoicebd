        ...
        <>
            ...
            <section className='container max-w-[1170px] min-h-screen mx-auto px-4'>
                ...
                <div className='w-full'>
                    {
                        (articleData.parents.length > 0) && (
                            <nav className='mt-6 flex items-center gap-8 pb-4 border-solid border-black border-b-[1px]'>
                                <h1 className='text-[#fd0408] text-[22px] font-bold'><Link href={`/category/${articleData.category_url}`}>{articleData.category}</Link></h1>
                                <ul className='flex items-center'>
                                    {articleData.parents.map((parent, i) => (
                                        <li
                                            key={i}
                                            className={`text-lg px-4 ${i < articleData.parents.length - 1 ? "border-solid border-black border-0 border-r-[1px]" : ""}`}
                                        >
                                            <NavLink path={`/category/জাতীয়/${parent.category_url}`} title={parent.category} />
                                        </li>
                                    ))}
                                </ul>
                            </nav>
